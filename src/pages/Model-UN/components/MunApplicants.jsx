...
export const MunApplicants = () => {
  const columns = [
    // ... (your existing columns)
    {
      ...
      renderCell: (params) => (
        <div style={{ display: 'flex', alignItems: 'center' }}>
          <div style={{ height: "35px", width: "45px" }}><img className='h-100 w-100' src={params.row.avatarUrl} size="40" round={true} /></div>
    ...
    },
    {
      ...
      renderCell: (params) => (
        <div style={{ display: 'flex', alignItems: 'center' }}>
          <span className='fw-400 fs-13-5px text-wrap' style={{ color: "#000000", fontFamily: "Lexend" }}>{params.row.class}</span>
        </div>
      ),
    ...
    {
      field: 'Preferred Committee',
      headerName: 'Preferred Committee',
      width: 180,
      renderCell: (params) => (
        <div style={{ display: 'flex', alignItems: 'center' }}>
          <span className='fw-400 fs-13-5px text-wrap' style={{ color: "#000000", fontFamily: "Lexend" }}>{params.row.preferredComm ? params.row.preferredComm :"-"}</span>
        </div>
      ),
      headerClassName: 'blue-header',
    ...
    },
    {
      ...
      headerName: 'Preferred Country',
      width: 180,
      renderCell: (params) => (
        <div style={{ display: 'flex', alignItems: 'center' }}>
          <span className='fw-400 fs-13-5px text-wrap' style={{ color: "#000000", fontFamily: "Lexend" }}>{params.row.preferredCountry ? params.row.preferredCountry : "-"}</span>
        </div>
      ),
      headerClassName: 'blue-header',
    ...
    },
    {
      ...
      renderCell: (params) => (
        <div style={{ display: 'flex', alignItems: 'center' }}>
          <span className='text-wrap fw-400 fs-13-5px text-wrap' style={{ color: "#000000", fontFamily: "Lexend" }}>{params.row.preferredRole ? params.row.preferredRole : "-"}</span>
        </div>
      ),
      headerClassName: 'blue-header',
    ...
    },
    {
      ...
      renderCell: (params) => (
        <div style={{ display: 'flex', alignItems: 'center' }}>
          <span className='fw-400 fs-13-5px text-wrap' style={{ color: "#000000", fontFamily: "Lexend" }}>{params.row.dateofApplication}</span>
        </div>
      ),
      headerClassName: 'blue-header',
    ...
    },
    {
      ...
      renderCell: (params) => (
        <div style={{ display: 'flex', alignItems: 'center' }}>
          <span className='fw-400 fs-13-5px text-wrap' style={{ color: "#000000", fontFamily: "Lexend" }}>{params?.row?.status}</span>
        </div>
      ),
      headerClassName: 'blue-header',
        ...
        <>
          ...
          <div className="dropdown-menu border-0 rounded-3" style={{boxShadow: "rgba(136, 165, 191, 0.48) 6px 2px 16px 0px, rgba(255, 255, 255, 0.8) -6px -2px 16px 0px"}}>
            <div className="d-flex flex-column ">
              ...
              <input type="checkbox" id="secure" className="mx-2" checked={isChecked}
...
// ============= Get all Applicants details  api ===============\\
  ...
  const filteredRows = activeFilter === "all" ? allApplicants : allApplicants?.filter(row => row.status === activeFilter);
  ...
  //============ Secure nomination handler  ====================\\

  const handlePreferredCommitee =async(studentId,registerId)=>{
    Swal.fire({
      title: 'Are You Sure ? ',
      text: 'You Want to secure nominate this applicant.',
      icon: 'warning',
      ...
      if (isConfirmed) {
    try {
       const assignRes= await apiJsonAuth.put(`/api/v2/modelUn-institute/nominateStudent/student/${studentId}/register/${registerId}`,{status:"approved",is_participant:true})
       if(assignRes?.status===200){
        toast.success("User Nomination successfully..")
         getAllApplicants();
       }
    } catch (error) {
      ...
      <div className="col-10 mx-auto mt-4">
        <div className="row">
          ...
          <div className='mt-3 d-flex justify-content-between'>
            <div><span className='fs-19px fw-600' style={{ color: "#32009C", fontFamily: 'Inter' }}>Nomination is set to be declared on {moment(eventDetail?.date_proposed, "YYYY/MM/DD").format("DD MMM, YYYY")}</span>
              <span className='fs-19px fw-600 ms-3' style={{ color: "#8F00FF", fontFamily: 'Inter' }}>Change Date</span>
            </div>
          ...
          <div className='d-flex justify-content-between'>
            <div className='d-flex justify-content-between ms-3 col-5'>
              {['All', 'Pending', 'Hold', 'Approved', 'Rejected'].map(filter => (
                <div key={filter}>
                  <span
            ...
            <div>
              <span className='fw-400 fs-17-50px' style={{ color: "#001AFF", fontFamily: "Lexend" }}>View Available Role</span>
            </div>
          </div>
        </div>
        {/* <hr className='mt-4'/> */}
